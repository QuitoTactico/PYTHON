...
def movy(m:list,n:int,d:bool) -> list:
    temp,rng,it,bl=m[-1][n-1] if d else m[0][n-1],range(len(m)-1,-1,-1) if d else range(len(m)),-1 if d else +1,0 if d else len(m)-1
    for i in rng: m[i][n-1] = m[i+it][n-1] if i != bl else temp
    return m
