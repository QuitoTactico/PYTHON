...
def list_to_str(data: list, upper_comma: bool) -> str:
    return (
        str(data)[1:-1] if upper_comma == False else str(data)[1:-1].replace("'", "`")
    )


def print_insert(insert_str, instance_data) -> None:
    output_file.write(
        insert_str + list_to_str(instance_data, False).replace("''", "null") + ");\n"
    )
...
with open("csv_to_mysql_INPUT_2.csv", "r", encoding="utf-8") as fichero:
    for linea in fichero:
        ...
        if mode == 1:
            fields = [
                i.lower().replace("ñ", "nn").replace(" ", "_") for i in data[:num_data]
            ]
