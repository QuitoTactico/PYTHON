...
def generate_bc(url: str, separator: str):
    ...
    names_list = [
        item.upper().replace("-", " ") if len(item) < 30 else acronymize(item)
...
def generate_bc(url, separator):
    ...
    url = (
        "".join(url.strip("/").rpartition("//")[2].partition("/")[1:])
        .rsplit("?", 1)[0]
        .rsplit("#", 1)[0]
        .rsplit(".", 1)[0]
        .rsplit("/index")[0]
    ...
    return separator.join(
        [a % ("/".join(url[:i]), bc(m)) for i, m in enumerate(url[:-1], 1)]
        + [span % bc(url[-1])]
    )
...
def generate_bc(url: str, separator: str) -> str:
    ...
    def format_bc(part: str, max_length: int = 30, home: str = "HOME") -> str:
        ...
        if len(formatted) > max_length:
            formatted = "".join(
                word[0] for word in formatted.split() if word not in restricted
            )
        return formatted

    *path, active_class = (
        "".join(
            url.strip("/").rpartition("//")[2].partition("/")[1:]  # drop protocol
        )  # drop host
    ...
    return separator.join(
        [a % ("/".join(path[:i]), format_bc(m)) for i, m in enumerate(path, 1)]
