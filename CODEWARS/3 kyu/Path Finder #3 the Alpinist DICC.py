...
def path_finder(maze):
    ...
    matrix_maze_beta = {
        i: {j: item for j, item in enumerate(list(map(int, list(line))))}
    ...
    min_L = 1
    L_list = [matrix_maze[i][0] for i in range(len_y)] + [
        matrix_maze[len_y - 1][i] for i in range(len_x)
    ]
    last = L_list[0]
    for i in L_list[1:]:
    ...
    min_J = 1
    J_list = [matrix_maze[0][i] for i in range(len_x)] + [
        matrix_maze[i][len_x - 1] for i in range(len_y)
    ]
    last = J_list[0]
    for i in J_list[1:]:
