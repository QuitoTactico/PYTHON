def f(M:list, x:int, y:int, k:int, d:int, cont:int) -> int : 
    if(M[y][x]=="#"): cont = cont + k
    if(M[y][x]=="*"): cont = cont + d
    if((x==len(M[0])-1)and(y==len(M)-1)): return cont
    if(x==len(M[0])-1): return f(M,x,y+1,k,d,cont)
    if(y==len(M)-1): return f(M,x+1,y,k,d,cont)
