...
def path_finder(maze):
    ...
    N = len(maze)
    directions = [(0, 1), (1, 0), (0, -1), (-1, 0)]  # Movimientos posibles: Este, Sur, Oeste, Norte
    
    def dijkstra():
        pq = [(0, 0, 0)]  # Cola de prioridad inicial con (costo, x, y)
        visited = [[False] * N for _ in range(N)]  # Matriz de nodos visitados
        costs = [[float('inf')] * N for _ in range(N)]  # Matriz de costos inicializada con infinito
        ...
        while pq:
            ...
            for dx, dy in directions:
                nx, ny = x + dx, y + dy
                if 0 <= nx < N and 0 <= ny < N and not visited[nx][ny]:  # Asegurarse de que el nodo es válido y no visitado
                    new_cost = cost + abs(maze[nx][ny] - maze[x][y])  # Calcular el nuevo costo, varía según ejercicio
                    if new_cost < costs[nx][ny]:  # Si el nuevo costo es menor, actualizarlo
                        costs[nx][ny] = new_cost
                        heapq.heappush(pq, (new_cost, nx, ny))  # Añadir el nodo a la cola de prioridad
